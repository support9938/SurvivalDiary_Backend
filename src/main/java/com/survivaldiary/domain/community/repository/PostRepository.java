...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("select p from Post p where p.category = :category and p.user.role = :userRole order by p.createdAt desc, p.id desc")
    ...
    @Query(value = "select p.* from posts p join users u on u.user_id = p.user_id "
            ...
            countQuery = "select count(*) from posts p join users u on u.user_id = p.user_id where u.role = 'USER'",
            nativeQuery = true)
